...
import { getJsonData, getLocalStorage } from 'c/cBUtilities';
...
export default class CBChangePassword extends NavigationMixin(LightningElement) {
    ...
    changePasswordHandler() {
        ...
        changePassword({ reqWrapper: reqWrapper })
            .then((result) => {
                ...
                this.authenticate(this.failureGif, "Could not change the password due to some tehcnical issue.", openConfirmModal);
            })
    ...
    */
    ...
    comfirmModalConf = {
        title: PASSWORD_CHANGED_MESSAGE,     // Title of the confirmation modal
        ...
        yesButton: {
            ...
            implementation: () => {     // Function to execute when the 'OK' button is clicked
                this.navigateToProfileSettings();   // Navigate to profile settings page
    ...
    getLocalStorageDetails() {
        this.sesisonId = getLocalStorage("CBSessionId") ? getLocalStorage("CBSessionId") : ''
        this.formId = getLocalStorage("CBFormId") ? getLocalStorage("CBFormId") : ''
        this.apiHeaders["Cookie"] = this.sesisonId ? this.sessionId : ''
    }
